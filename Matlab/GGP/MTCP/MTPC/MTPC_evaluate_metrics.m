...
%     stats(m).p_orig1=reshape(p_all_temp(:,1,:,:,:,:,:,:,:),n_terms,n_thresh,36);
%     stats(m).t_orig1=reshape(t_all_temp(:,1,:,:,:,:,:,:,:),n_terms,n_thresh,36);
%     stats(m).f_orig1=reshape(f_all_temp(:,1,:,:,:,:,:,:,:),n_terms,n_thresh,36);
%     stats(m).f_crit=reshape(f_crit_temp,n_terms,n_thresh,36);
%     stats(m).p_corr1=reshape(p_corr_temp,n_terms,n_thresh,36);
    
%     stats(m).p_orig1=squeeze(p_all_temp(:,1,:,:,:,:,:,:,:));
%     stats(m).t_orig1=squeeze(t_all_temp(:,1,:,:,:,:,:,:,:));
%     stats(m).f_orig1=squeeze(f_all_temp(:,1,:,:,:,:,:,:,:));
%     stats(m).f_crit=squeeze(f_crit_temp);
%     stats(m).p_corr1=squeeze(p_corr_temp);
    ...
    if strcmp(config.method,'mtpc') | strcmp(config.method,'mtpc+scauc') | strcmp(config.method,'scauc')
        ...
        [mat_temp, tidx_corr2]=nanmax(abs(stats(m).t_orig1),[],2);
        stats(m).max_t_thresh_idx=tidx_corr2;
...
% do a faster t-test
%[~,p,~,s]=ttest2(gt(1:13),gt(14:end));

%t_temp(1) = s.tstat;
%f_temp(1) = t_temp^2;
%p_temp(1) = p;

%coefnames = {'(Intercept)','x1'};
%mdl = [];
%tbl = [];
