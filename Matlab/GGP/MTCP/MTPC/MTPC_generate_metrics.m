...
%           which provide an alterantive connectivity matrices used for weights (e.g. is
%           one wishes to weight edges by FA,  but wants to use streamline count to threhsold). 
...
% keep_CMthresh: 0 or 1, specifes is output cnfig structure should contain
...
% where do  waights for the graph coming from?
if isnumeric(config.weights)
    if ~isequal(size(config.weights),size(CM));
        error('Alternative weight-definitions must be an array mathcing size of CM.');
    end
    CMw=config.weights;
...
% set default sets of BCT functions to use (note these defualts assume  undirected matrices!!!)
...
for s=1:n_subj
    ...
    % absolut thresholds correspond to a relative threshold (relative to
    % largest value in CM)
