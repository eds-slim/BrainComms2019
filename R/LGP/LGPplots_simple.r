...
ggsave(file.path(OUTPUT_DIR,"final","Figures",paste("LGP_",plot.LGP,"100.png",sep = "")), width=10, height = 7, units = 'in')

tikz(file = file.path(OUTPUT_DIR,"final","Figures",paste("LGP_",plot.LGP,"100.tex",sep = "")),width=7.28346,height=6, standAlone = TRUE);
print(p2+theme(legend.position = "none")+annotation_custom(grob=ggplot_gtable(pg), xmin=6, ymin=7));
dev.off();
}

file.copy(file.path(OUTPUT_DIR,"final","Figures",paste("LGP_","efficiency","100.png",sep = "")),file.path(OUTPUT_DIR,"final","Figures","LGP.png"), overwrite = TRUE)
file.copy(file.path(OUTPUT_DIR,"final","Figures",paste("LGP_","efficiency","100.tex",sep = "")),file.path(OUTPUT_DIR,"final","Figures","LGP.tex"), overwrite = TRUE)
